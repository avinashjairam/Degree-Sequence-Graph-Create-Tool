...
class NewPanel extends JPanel implements ActionListener{
  ...
  int [] candidates;
    ...
    public void initializeSequence(){
      ...
      candidates = new int [data.length()];
  ...
  public void findEdges(){
    ...
    int lastJ =-1;
      ...
      while(checks < originalSequence[i]){
        int x =rand.nextInt(list.size());
        matrix[i][list.get(x)] = 1;
        list.remove(x);
        checks++;
      }
 ...
 public void decider(int k){
  System.out.print("decider candidates");
  for(int i =0; i<size;i++){
    if(i == k){
      continue;
    }
    else if(originalSequence[i] < countEdges(i)){
      candidates[i]=i;
      System.out.print(i);
    }
  }
